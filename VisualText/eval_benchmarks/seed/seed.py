...
class TrainDataset(Dataset):
    ...
    def init_dataset(self):
        ...
        print('evaluation on SEED benchmark')
        import os
        from os.path import join,exists
        data_root = 'SEED-Bench-image/'
        my_path='SEED-Bench.json'

        with open(my_path,'r') as f:
            samples = json.load(f)
        samples=samples['questions']

        all_samples_len= len(samples)

        for index in range(all_samples_len):
            sample=samples[index]
            answer = sample['answer']
            opt_a=sample['choice_a']
            opt_b=sample['choice_b']
            opt_c=sample['choice_c']
            opt_d=sample['choice_d']
            image_path=data_root+sample['data_id']
            question=sample['question']

            instruction = 'This is an image:\n<image_start><image><image_end>. Question: '+ question+'\nOptions:'
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            try:
                image_path=image_path.replace('SEED-Bench-image','videos')
...
if __name__ == '__main__':
    ...
    results_dir='eval_results/SEED/'+name_tag
