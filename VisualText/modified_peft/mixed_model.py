...
class PeftMixedModel(PushToHubMixin, torch.nn.Module):
    ...
    def forward(self, *args: Any, **kwargs: Any):
        ...
        """
        # Check if adapter_names is provided in kwargs
        adapter_names = kwargs.pop("adapter_names", None)
        
        if adapter_names is not None:
            # If adapter_names is provided, use the _enable_peft_forward_hooks context
            with self.base_model._enable_peft_forward_hooks(*args, **kwargs):
                return self.base_model(*args, **kwargs)
        else:
            # Normal forward pass without adapter_names
            return self.base_model(*args, **kwargs)
